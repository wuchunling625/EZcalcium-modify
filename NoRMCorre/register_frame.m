function [shifts,Mf] = register_frame(Yt,fftTempMat,fftTempPatches,patches,options,idx)
...
for i = 1:N_patches
    ...
    % rebuild: replace the shifts with previous shifts value
    if(~isempty(options.shift_g_in))
        shifts(ix,iy,iz,1) = options.shift_g_in(idx).shifts(ix,iy,iz,1);
        shifts(ix,iy,iz,2) = options.shift_g_in(idx).shifts(ix,iy,iz,2);
    end
end
